...
opciones=(
    ...
    "Mostrar particiones montadas:|df -h"

    "Mostrar tamaño de ficheros y directorios ordenados por tamaño:|ls -lSr | more"

    "Estimar espacio usado por un directorio:|du -sh dir1"

    "Mostrar tamaño de ficheros y subdirectorios en la ubicación actual:|du -sk * | sort -nr"

    "Mostrar tamaño de subdirectorios en la ubicación actual:|du -h --max-depth=1 | sort -nr"

    "Mostrar tamaño de directorios y archivos en GiB:|du -k --max-depth=1 | sort -k1 -nr | awk '{printf \"%.3f GiB\\t%s\\n\", \$1/(1048576), \$2}' | less"

    "Mostrar espacio usado por los paquetes rpm instalados organizados por tamaño (Fedora, Redhat y otros):|rpm -q -a --qf '%10{SIZE}t%{NAME}n' | sort -k1,1n"

    "Mostrar paquetes instalados que más espacio consumen (en orden descendente) en Debian o derivadas:|dpkg-query -W -f='\${Package}\t\${Installed-Size}\n' | sort -k 2 -nr | grep -v deinstall | head -n 25 | awk '{printf \"%.3f \t%s\\n\", \$2/(1024), \$1}'"
