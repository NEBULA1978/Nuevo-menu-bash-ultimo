...
opciones=(
    ...
    "Arquitectura de la máquina (1):|arch"
    "Arquitectura de la máquina (2):|uname -m"
    "Versión del kernel usado:|uname -r"
    "Información completa:|uname -a"
    "Información completa de la distribución:|lsb_release -a"
    "Nombre de la distribución:|cat /etc/issue"
    "Componentes (hardware) del sistema:|dmidecode -q"
    "Características de un disco duro:|hdparm -i /dev/hda"
    "Prueba de lectura en un disco duro:|hdparm -tT /dev/sda"
    "Información de la CPU:|cat /proc/cpuinfo"
    "Número de procesadores:|grep -c ^processor /proc/cpuinfo"
    "Interrupciones:|cat /proc/interrupts"
    "Uso de memoria:|cat /proc/meminfo"
    "Ficheros swap:|cat /proc/swaps"
    "Versión del kernel:|cat /proc/version"
    "Adaptadores de red y estadísticas:|cat /proc/net/dev"
    "Sistema de ficheros montado:|cat /proc/mounts"
    "Dispositivos PCI:|lspci -tv"
    "Dispositivos USB:|lsusb -tv"
    "Listar el hardware:|lshw"
    "Listar el hardware (alternativo):|discover"
    "Fecha del sistema:|date"
    "Almanaque de 2011:|cal 2011"
    "Almanaque para el mes julio de 2011:|cal 07 2011"
    "Colocar (declarar, ajustar) fecha y hora:|date 041217002011.00"
    "Guardar los cambios de fecha en la BIOS:|clock -w"
    "Información (nombre, etiqueta, UUID, tipo de partición) sobre los dispositivos de bloque (discos rígidos, etc.):|blkid"
    "Salir:|exit 0"
)
